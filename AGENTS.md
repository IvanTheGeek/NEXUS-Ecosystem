...
> Condensed from `NEXUS-LOGOS/docs/methods/paths.md`. Update that file AND this file together.

- A PATH is a **named, purposeful journey through the Event Model from a specific actor's perspective** — first-class design artifact, not documentation byproduct
- **PATHs are the source of all example data** — example data belongs to a PATH; no separate test data pool
- **Design and test are the same artifact** — PATH example data flows directly into GWT Given/When/Then; no "write tests later"
- Every PATH has: Name, Actor, Goal, Sequence of slices, Example data, Lifecycle status
- **PATH Lifecycle**: Identified → Modeled → Specified → Implemented → Verified
- **PATH Registry** — all known PATHs (designed, discovered via ATLAS, telemetry via PAXUS); grows continuously
- **Minimal Coverage Set** — curated subset of the registry; one PATH per unique branch; wired into Playwright; human-gated additions
- **PATH Chaining** — atomic PATHs compose into larger journeys; output state of PATH A = input context of PATH B; data gaps between chains are design problems surfaced early
- **Runtime PATHs** — the user's actual event log IS a PATH (captured by PAXUS); design PATHs = what should happen; runtime PATHs = what did happen; gap = where bugs live
- **Fidelity levels**: Executive (happy path, business needs) · UX (interactions) · Domain (workflow) · Technical (internals)
- **ATLAS** visualizes and explores PATHs: predefined playback, choose-your-own-adventure (dynamic chaining), linear walkthrough view
- **Literal vs. metaphorical**: literal PATHs traverse Event Model slices; navigation PATHs traverse content nodes (ivanthegeek.com) — same primitive, different substrate
...
> Condensed from `FSHARP.md`. Update that file AND this file (`AGENTS.md`) together.
...
- Railway-Oriented Programming: chain `Result`-returning functions with `result { }` CE or `>>=`; errors propagate without explicit checking at every step
